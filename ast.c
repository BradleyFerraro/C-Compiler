...
/* Define function stubs as per the prototypes in ast.h */
...
char *func_def_argname(void *ptr, int n) {
    ASTNode *astree = (ASTNode *)ptr;
    if (astree == NULL || astree->type != FUNC_DEF || n <= 0 || n > astree->args) {
        return NULL;
    }
    ASTNode *cur = astree->child1;
    for (int i = 1; i < n && cur != NULL; i++) {
        cur = cur->child1;
    }
    if (cur != NULL && cur->type == IDENTIFIER) {
        return cur->name;
    }
    return NULL;
}
...
void *func_def_body(void *ptr) {
    ASTNode *astree = (ASTNode *)ptr;
    if (astree != NULL && astree->type == FUNC_DEF) {
        return astree->child0;
    }
    return NULL;
}
...
char *func_call_callee(void *ptr) {
    ASTNode *astree = (ASTNode *)ptr;
    if (astree != NULL && astree->type == FUNC_CALL) {
        return astree->name;
    }
    return NULL;
}
...
void *stmt_list_head(void *ptr) {
    ASTNode *astree = (ASTNode *)ptr;
    if (astree != NULL && astree->type == STMT_LIST) {
        return astree->child0;
    }
    return NULL;
}
...
void *expr_list_head(void *ptr) {
    ASTNode *astree = (ASTNode *)ptr;
    if (astree != NULL && astree->type == EXPR_LIST) {
        return astree->child0;
    }
    return NULL;
}
...
char *expr_id_name(void *ptr) {
    ASTNode *astree = (ASTNode *)ptr;
    if (astree != NULL && astree->type == IDENTIFIER) {
        return astree->name;
    }
    return NULL;
}
...
int expr_intconst_val(void *ptr) {
    ASTNode *astree = (ASTNode *)ptr;
    if (astree != NULL && astree->type == INTCONST) {
        return astree->args;
    }
    return 0;
}
...
void *expr_operand_1(void *ptr) {
    ASTNode *astree = (ASTNode *)ptr;
    if (astree != NULL && (astree->type == ADD || astree->type == SUB || 
                           astree->type == MUL || astree->type == DIV || 
                           astree->type == UMINUS || astree->type == AND || 
                           astree->type == OR || astree->type == EQ || 
                           astree->type == NE || astree->type == LE || 
                           astree->type == LT || astree->type == GE || 
                           astree->type == GT)) {
        return astree->child0;
    }
    return NULL;
...
void *expr_operand_2(void *ptr) {
    ...
    if (astree != NULL && (astree->type == ADD || astree->type == SUB || 
                           astree->type == MUL || astree->type == DIV || 
                           astree->type == AND || astree->type == OR || 
                           astree->type == EQ || astree->type == NE || 
                           astree->type == LE || astree->type == LT || 
                           astree->type == GE || astree->type == GT)) {
        return astree->child1;
    }
    return NULL;
}
...
char *stmt_assg_lhs(void *ptr) {
    ASTNode *astree = (ASTNode *)ptr;
    if (astree != NULL && astree->type == ASSG) {
        return astree->child0->name;
    }
    return NULL;
}
...
void *stmt_while_expr(void *ptr) {
    ASTNode *astree = (ASTNode *)ptr;
    if (astree != NULL && astree->type == WHILE) {
        return astree->child0;
    }
    return NULL;
}
...
void *stmt_return_expr(void *ptr) {
    ASTNode *astree = (ASTNode *)ptr;
    if (astree != NULL && astree->type == RETURN) {
        return astree->child0;
    }
    return NULL;
}
